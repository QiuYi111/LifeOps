...
在这个系统中，**以GIT仓库为准**。所有的日程数据都以 JSON 格式存储在私有仓库中。你不需要打开复杂的日历应用，只需要在 **飞书 (Feishu/Lark)** 中与 AI 机器人对话，它就会利用 **GitHub Actions** 的边缘计算能力，驱动 **Claude Code + GLM4.6** 进行智能决策、冲突检测和日程排期，并最终通过飞书机器人 **邀请同步** 到你的日历。
